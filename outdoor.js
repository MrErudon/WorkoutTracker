...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const gpsDetails=document.createElement('details');gpsDetails.id='gpsDetails';gpsDetails.className='hybrid-note';gpsDetails.innerHTML='<summary>Use live GPS instead</summary>';gpsDetails.append(section);document.getElementById('panel-run').append(gpsDetails);if(outdoor)gpsDetails.open=true;
  document.querySelector('#panel-run .workout-header').insertAdjacentHTML('afterend','<p class=backup-note>Log your time after you finish — no GPS or open screen needed. Use minutes (45), mm:ss (45:30), or hh:mm:ss (1:15:00). Calories are rough estimates.</p>');document.getElementById('gpsBody').value=goals.weight||210;
  ...
  if(outdoor){gpsDetails.open=true;outdoor.running=false;document.getElementById('gpsKind').value=outdoor.kind;document.getElementById('gpsBody').value=outdoor.bodyLbs;document.getElementById('gpsPack').value=outdoor.packLbs;outdoorMessage='Saved session recovered and paused. Resume to continue GPS tracking.';}
